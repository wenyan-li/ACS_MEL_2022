...
p11 <- LabelPoints(plot = p10, points = top10, repel = TRUE)
p10 + p11

# 10X Seurat
```

### 3c. Check how many PCs to use for UMAP and clustering
...
### 3d. Play with UMAP and cluster parameters
```{r}
# umap
...
rhap <- RunUMAP(
                ...
                min.dist = 0.3,          # 0.001 - 0.5 how tightly the points to be. Higher values to get more evenly distribution.
...
### 3e. Differential gene expression
...
rhap.markers <- FindAllMarkers(rhap, 
                               only.pos = TRUE, # only calculate the up-regulated genes. Set it to FALSE to get down-regulated genes
                               )
